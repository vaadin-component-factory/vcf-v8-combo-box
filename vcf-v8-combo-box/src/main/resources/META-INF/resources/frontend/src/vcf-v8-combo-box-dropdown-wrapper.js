...
import './vcf-v8-combo-box-item.js';
import './vcf-v8-combo-box-dropdown.js';
...
class ComboBoxDropdownWrapperElement extends (class extends PolymerElement {}) {
  static get template() {
    return html`
    <vcf-v8-combo-box-dropdown id="dropdown" hidden="[[_hidden(_items.*, loading)]]" position-target="[[positionTarget]]" on-template-changed="_templateChanged" on-position-changed="_setOverlayHeight" disable-upgrade="" theme="[[theme]]">
      <template>
        <style>
          #scroller {
            overflow: auto;
        ...
        <div id="scroller" on-click="_stopPropagation">
          <iron-list id="selector" role="listbox" items="[[_getItems(opened, _items)]]" scroll-target="[[_scroller]]">
            <template>
              <vcf-v8-combo-box-item on-click="_onItemClick" index="[[__requestItemByIndex(item, index, _resetScrolling)]]" item="[[item]]" label="[[getItemLabel(item, _itemLabelPath)]]" selected="[[_isItemSelected(item, _selectedItem, _itemIdPath)]]" renderer="[[renderer]]" role\$="[[_getAriaRole(index)]]" aria-selected\$="[[_getAriaSelected(_focusedIndex,index)]]" focused="[[_isItemFocused(_focusedIndex,index)]]" tabindex="-1" theme\$="[[theme]]">
              </vcf-v8-combo-box-item>
            </template>
          </iron-list>
        </div>
      </template>
    </vcf-v8-combo-box-dropdown>
`;
  }

  static get is() {
    return 'vcf-v8-combo-box-dropdown-wrapper';
  }
